...
为分析不同序列长度对各方法性能的影响, 在$T=16/32/64$下进行了对比实验, 结果如表~3所示.

\begin{center}
{\CJKfamily{kai} 表~3~~不同序列长度下的性能对比\\
Table~3~~Performance comparison under different sequence lengths}\\
\label{tab:3} \vskip 3pt
{\fontsize{9.3pt}{11.6pt}\selectfont
\begin{tabular}{@{ }lcccc@{ }}\toprule
& \multicolumn{2}{c}{MSE} & \multicolumn{2}{c}{推理时间/ms}\\
\cmidrule(l){2-5}
$T$ & SSM-WM & LSTM-WM & SSM-WM & LSTM-WM\\
\midrule
16 & 0.004735 & 0.001364 & 18.4 & 15.9\\
32 & 0.004414 & 0.001261 & 16.9 & 4.1\\
64 & 0.004181 & 0.001171 & 18.5 & 30.0\\
\bottomrule
\end{tabular}}\end{center}

结果表明:
\begin{enumerate}
\item[1)] SSM-WM的推理时间几乎不随序列长度增加而增长(18.4ms$\to$18.5ms),
体现了$O(T\log T)$计算复杂度的优势;
\item[2)] LSTM-WM的推理时间随序列长度线性增长(15.9ms$\to$30.0ms),
在$T=64$时SSM-WM已实现1.6倍加速;
\item[3)] 随着序列长度进一步增加, SSM-WM的速度优势将更加显著.
\end{enumerate}
