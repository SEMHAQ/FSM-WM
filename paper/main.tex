...
MIMO-WM的完整前向推理过程为: 首先将状态-动作序列编码至隐空间, 然后依次通过$L$层MIMO块提取特征, 最后解码预测下一时刻状态. 具体而言, 编码得到${\bm z} = \text{Encoder}([{\bm s}_{0:T{-}1}; {\bm a}_{0:T{-}1}])$;
对每层MIMO块依次执行归一化$\tilde{{\bm z}} = \text{LayerNorm}({\bm z})$、并行SSM变换${\bm h} = \text{DiagSSM}(\tilde{{\bm z}})$、门控融合${\bm o} = {\bm h} \odot \sigma({\bm W}_g\tilde{{\bm z}} + {\bm b}_g)$和残差更新${\bm z} = {\bm z} + {\bm W}_o{\bm o} + {\bm b}_o$;
最终解码输出$\hat{{\bm s}}_T = {\bm s}_{T{-}1} + \text{Decoder}({\bm z}_{T{-}1})$.
...
当$\sigma \to {\bm 0}$时SSM输出被抑制, $\sigma \to {\bm 1}$时退化为无门控SSM.
...
CEM-MPC首先初始化采样分布${\bm \mu} = {\bm 0}$, ${\bm \sigma} = {\bm 1}$,
然后进行$M$轮迭代: 每轮从$\mathcal{N}({\bm \mu}, \diag({\bm \sigma}^2))$采样$K$个候选动作序列并截断至$[-1, 1]$, 用MIMO-WM前向展开$H$步计算各序列代价$J^{(k)}$, 选取代价最小的$N_e$个精英样本更新分布参数
\begin{equation}
{\bm \mu} = \frac{1}{N_e}\sum_{{\bm a} \in E} {\bm a}, \quad
{\bm \sigma} = \sqrt{\frac{1}{N_e}\sum_{{\bm a} \in E} ({\bm a} - {\bm \mu})^2} + \epsilon.
\end{equation}
迭代结束后输出${\bm a}_T^* = {\bm \mu}[0]$作为最优动作.
$K$个候选序列的评估通过GPU批量并行实现,
